...
import { CartCheckoutData } from "../card/CartCheckoutData";
import { settings } from "../../helpers/settings";
import { Icon } from "../ui/Icon";
import { removeIsCheckout } from "../../redux/slices/layoutSlice";
...
export const CartCheckoutDataWrapper = () => {
  const { user } = useSelector((state) => state);

  const dispatch = useDispatch();

  return (
    <div className="cart-checkout-data__wrapper">
      <div className="content__row">
        <div className="title__wrapper">
          <h2 className="title">Checkout</h2>
          <p className="description">Confirme su pedido</p>
        </div>
        <Icon onClick={() => dispatch(removeIsCheckout())}>{closeIconSvg}</Icon>
      </div>
      <div className="cart-checkout-data__list">
        {settings.map((userData) => (
          <CartCheckoutData
            title={userData.title}
            data={user[userData.title]}
            type="text"
          />
        ))}
      </div>
